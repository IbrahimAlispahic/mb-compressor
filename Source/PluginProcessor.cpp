...
//    compressor.process(buffer);
    ...
    auto addFilterBuffer = [nc = numChannels, ns = numSamples](auto& inputBuffer, const auto& source) {
        for(int i = 0; i < nc; ++i) {
            inputBuffer.addFrom(i, 0, source, i, 0, ns);
        }
    };
    
    addFilterBuffer(buffer, filterBuffers[0]);
    addFilterBuffer(buffer, filterBuffers[1]);
}
...
juce::AudioProcessorValueTreeState::ParameterLayout MBCompAudioProcessor::createParameterLayout() {
    ...
    layout.add(std::make_unique<AudioParameterFloat>(params.at(Names::Threshold_Low_Band),
                                                     ...
                                                     0));
    // min attack and release time of 5ms, compressor will take at least 5ms to start compressing once the signal goes over the threshold
    ...
    // Adding ratios- 1:1 means no ratio, 1:100 brick wall ratio. Those two will be used as start and end
    auto choises = std::vector<double>{ 1, 1.5, 2, 3, 4, 5, 6, 7, 8, 10, 15, 20, 50, 100 };
    juce::StringArray sa;
    for(auto choice : choises) {
